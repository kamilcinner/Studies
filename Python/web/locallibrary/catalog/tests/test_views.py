...
# Required to assign User as a borrower and to grant the permission needed to set a book as returned.
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_redirect_if_logged_in_but_not_correct_permission(self):
        login = self.client.login(username='testuser1', password='1X<ISRUkw+tuK')
        response = self.client.get(reverse('catalog:renew-book-librarian', kwargs={'pk': self.test_bookinstance1.pk}))
        self.assertEqual(response.status_code, 302) # Was 403 here
...
class AuthorCreateTest(TestCase):
    ...
    def test_redirect_if_logged_in_but_not_correct_permission(self):
        login = self.client.login(username='testuser1', password='1X<ISRUkw+tuK')
        response = self.client.get(reverse('catalog:author_create'))
        self.assertEqual(response.status_code, 403) # But here is giving 403 instead of 302 (probably because it's django default ModelView
    ...
    def test_form_date_of_death_initially_has_date_04_02_2020(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('catalog:author_create'))
        self.assertEqual(response.status_code, 200)

        date_04_02_2020 = datetime.date(2020, 2, 4)
        self.assertEqual(response.context['form'].initial['date_of_death'], date_04_02_2020)

    def test_redirects_to_author_detail_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        valid_first_name = 'Kamil'
        valid_last_name = 'Jaskot'
