            ...
            for operation in operations:
                ...
                for i in range(0, rotations):
                    if alg[:3] == 'rsa':
                        ...
                        is_public = '-pubin' if operation == 'encrypt' else ''

                        key_file = f'./keys/{key_type}.{alg[3:]}.key'

                        cmd += f'{openssl} rsautl -in {name} -out {temp_out} {is_public} -inkey {key_file} -{operation} 2> /dev/null; '
                    else:
                        key = '0123456789abcdef'
                        op_type = operation[0]
    
                        cmd += f'{openssl} enc -{alg} -{op_type} -in {name} -out {temp_out} -K {key} -iv {key} 2> /dev/null; '
                cmd = cmd[:-2]
                cmd += '\''
...
def calculate_bytes_per_sec(seconds, size, rotations):
    one_rotation_time = seconds / rotations
    return size / one_rotation_time
        ...
        for name in time:
            for alg in time[name]:
                for operation in time[name][alg]:
                    ...
                    f.write(f'{name}  \t{operation}\t{alg}\tBps:\t{seconds}\n')
    ...
    with open(result_file, 'w') as f:
        for alg in average:
            for operation in average[alg]:
                f.write(f'{operation}\t{alg}\tBps:\t{average[alg][operation]}\n')


def merge_time(encrypt_time, decrypt_time):
    for file in encrypt_time:
        for alg in encrypt_time[file]:
            encrypt_time[file][alg]['decrypt'] = decrypt_time[file][alg]['decrypt']
...
def main():
    ...
    # merge_time(time, do_time(files, algorithms, False))
