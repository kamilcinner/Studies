var haslo = "";
function letsBegin() {
    var pass = document.getElementById("pole").value;
    location.replace("szubienica.html");
    haslo = pass;
}
...
function sprawdz(nr) {
    ...
    //wygrana
    if (haslo == haslo1) {
        document.getElementById("alfabet").innerHTML = "Tak jest! Podano prawidłowe hasło: " + haslo + '<br/><br/><span class=reset onclick="location.replace(\'index.html\');">JESZCZE RAZ?</span>';
    }
    //przegrana
    if (ileSkuch >= 9) {
        document.getElementById("alfabet").innerHTML = "Przegrana! Prawidłowe hasło: " + haslo + '<br/><br/><span class=reset onclick="location.assign("index.html")">JESZCZE RAZ?</span>';
    }
}