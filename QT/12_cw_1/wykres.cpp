...
void Wykres::setGraphDist() {
    xGraphDist = static_cast<int>(20 * scalingParam);
    yGraphDist = static_cast<int>(20 * scalingParam);
    if(qAbs(minX) > maxX) xGraphDist += qAbs<int>(minX);
    else xGraphDist += maxX;
    if(qAbs(minY) > maxY) yGraphDist += qAbs<int>(minY);
    else yGraphDist += maxY;
}

void Wykres::drawXaxis() {
    QGraphicsTextItem *text;
    scene.addLine(- xGraphDist, 0, xGraphDist, 0, axisPen);
    text = scene.addText("osX[-]");
    text->setPos(xGraphDist + 5, - 12);
}

void Wykres::drawYaxis() {
    QGraphicsTextItem *text;
    scene.addLine(0, - yGraphDist, 0, yGraphDist, axisPen);
    text = scene.addText("osY[-]");
    text->setPos(- 17, - yGraphDist - 25);
}
