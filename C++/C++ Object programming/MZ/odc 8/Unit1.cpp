...
{
        if(Application->MessageBoxA("Czy na pewno utworzyæ nowy plik?","PotwierdŸ",MB_YESNOCANCEL | MB_ICONQUESTION)==IDYES)
        {
                tresc->Lines->Clear();
                nazwapliku="";
        }
...
//---------------------------------------------------------------------------
void __fastcall TForm1::Zako1Click(TObject *Sender)
{
        if(Application->MessageBoxA("Czy na pewno zakoñczyæ program?","PotwierdŸ",MB_YESNO | MB_ICONQUESTION)==IDYES)
        {
...
{
        if(Application->MessageBoxA("Czy na pewno zakoñczyæ program?","PotwierdŸ",MB_YESNO | MB_ICONQUESTION)==IDNO)
        {
                Action=caNone;
        }
}
...
//---------------------------------------------------------------------------
void __fastcall TForm1::Cz1Click(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TForm1::Stronka1Click(TObject *Sender)
{
        ShellExecute(NULL,"open","http://purepc.pl",NULL,NULL,SW_SHOWNORMAL);
}
//---------------------------------------------------------------------------
